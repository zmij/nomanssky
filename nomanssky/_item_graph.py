...
class _NodeContainer(Generic[T]):
    ...
    def __init__(self, walk_order: WalkOrder, log: Callable[[str], None]) -> None:
        ...
        self._queue = _WALK_CONTAINERS[walk_order][_NodeContainer._Node[T]]()
    ...
    async def add(
        ...
        for item in items:
            if not item in self:
                await visitor.discover_node(item, distance)
